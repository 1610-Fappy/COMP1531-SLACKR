...
from channel_for_testing import channel_join, channel_details
from message_for_testing import message_send, message_remove
import pytest
...
def test_message_remove():
    ...
    # User 1 is able to delete a message in Channel 1 because they are the owner
    assert(message_remove(user1_token, channel_1) == {})
...
def test_message_remove_except1a():
    ...
    # -----------------------------------------> just to activate a raise: remove later <-----------------------------------------
    ID_of_message = 256
...
def test_message_remove_except2a():
    ...
    # Sends a message in Channel 1
    ID_of_message = message_send(user1_token, channel_1, test_message)

    # User 2 tries to remove a message in Channel 1 but has not joined
...
#   AccessError if User 2 is part of Channel 1, but did not create the message or is an admin
def test_message_remove_except2b():
    ...
    # User 2 tries to remove a message in Channel 1 but is not an admin or owner
    auth_register('C00LGUY@hotmail.com', 'aVeryC00lguy', 'Bruce', 'Willis')
    ...
    # channel_1_possible_owner = channel_1_owners[1]
