...
def testing_channel_invite():
    ...
    assert channel_invite(User1['Token'], 'Sicko Mode', User2['u_id'])
    assert channel_invite(User2['Token'], 'Sicko Mode', User3['u_id'])
    
    # Invalid if channel_id does not exist
    with pytest.raises(InputError) as e:
        channel_invite(User1['Token'], '-1', User4['u_id'])

    # Invalid if u_id does not exist
    with pytest.raises(InputError) as e:
        channel_invite(User1['Token'], Channel1['channel_id'], '-1')
